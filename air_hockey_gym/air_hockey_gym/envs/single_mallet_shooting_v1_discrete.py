...
class SingleMalletShootDiscreteEnvV1(MujocoEnv):
    """
    ...
    #@param mal1_box Bounding box which mallet 1 will spawn uniformly in.
    #       Format:[(x1,y1),(x2,y2)] Where (x1,y1) are coordinates of top left corner of box, and (x2,y2) is bottom right
    #       In world coordinate frame (center of table, positive x towards opponent goal)

    #@param mal2_box Bounding box which mallet 2 will spawn uniformly in.
    #       Format:[(x1,y1),(x2,y2)] Where (x1,y1) are coordinates of top left corner of box, and (x2,y2) is bottom right
    #       In world coordinate frame (center of table, positive x towards opponent goal)
    
    def __init__(self, max_reward=10, puck_box=[(-0.5,0), (-0.5,0)], mal1_box= [(-0.9,0),(-0.9,0)], mal2_box= [(0.05,0.45),(0.95,-0.45)], max_accel=5, **kwargs):
    ...
    def reset_model(self):
        ...
        m2_x = unif(self.m2_box[0][0], self.m2_box[1][0]) - 0.25
        m2_y = unif(self.m2_box[0][1], self.m2_box[1][1])
    ...
    def _get_obs(self):
        ...
        cart_p = np.concatenate((self.data.qpos[0:2], self.data.qvel[0:2]))
    	#Mallet 1's DOF are defined 4th and 5th in the XML
        cart_m1 = np.concatenate((self.data.qpos[3:5], self.data.qvel[3:5]))
        #Mallet 2's DOF are defined 7th and 8th in the XML
        cart_m2 = np.concatenate((self.data.qpos[6:8], self.data.qvel[6:8]))

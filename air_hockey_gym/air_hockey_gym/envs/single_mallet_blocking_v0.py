...
class SingleMalletBlockEnv(MujocoEnv):
	...
	### Observation 
	(3,4) numpy array containing positions and velocities of all objects (puck, mallet 1, mallet 2)
    ...
    def __init__(self, max_reward=10, puck_box=[(0.10,0.25), (0.55,-0.25)], mal2_x_offset = 0.4, 
        ...
        observation_space = Box(low=-np.inf, high=np.inf, shape=(3,4), dtype=np.float64)
    ...
    def step(self, a):
        ...
        #self._check_puck_bounced()
        #puck_stopped = self.data.qvel[0] < PUCK_STATIONARY_THRESH and self.data.qvel[1] < PUCK_STATIONARY_THRESH
        crossed_middle = self.data.qpos[0] > 0 and self.data.qvel[0] > 0
        terminated = goal_scored or crossed_middle or self.num_steps > 200
    ...
    def _get_rew(self, obs):
        ...
        if(self.data.qpos[0] > 0 and self.data.qvel[0] > 0):
            rew += 0.5 * self.data.qvel[0] * self.max_reward
